...
#include <condition_variable> // For worker thread synchronization
...
{   
    ...
    std::string settingsFilePath = ""; // Resolved path to settings file provided by ORB_SLAM3 package
    std::string settingsFileBasePath = ""; // Base path to settings directory (without filename)
    std::string settingsParamOverride = ""; // Either an absolute file path or directory provided via parameter
    std::string settingsDirMonocular = "";
    std::string settingsDirMonocularInertial = "";
    std::string defaultSettingsName = "smartrollerz";
    bool waitForHandshake = true;
    bool useExternalTimestep = false;
    ...
    std::string subImuMsgName = ""; // Topic to receive IMU measurements from the python node
    ...
    bool useInertialMeasurements = false;
    bool hasLastImageTimestamp = false;
    double lastImageTimestamp = 0.0;
    std::deque<ORB_SLAM3::IMU::Point> imuMeasurementBuffer;
    std::mutex imu_mutex_;

    //* Frame queue for async SLAM processing
    struct FrameData {
        cv::Mat image;
        double timestamp;
    ...
    std::mutex frameQueue_mutex_;
    std::condition_variable frameQueue_cv_;
    std::thread slamWorkerThread_;
    bool workerThreadRunning_ = false;
    bool workerThreadShouldStop_ = false;
    size_t frameQueueMaxSize_ = 30;  // Max queued frames to prevent unbounded memory growth
    size_t totalFramesDropped_ = 0;  // Metrics: count of dropped frames
    
    //* Frame processing metrics
    size_t totalFramesProcessed_ = 0;
    std::chrono::high_resolution_clock::time_point lastMetricsTime_;
    std::string lastProcessingMetrics_ = "";
    ...
    void Imu_callback(const sensor_msgs::msg::Imu& msg); // Callback to process IMU data sent by Python node
        ...
        void RunSlamProcessing(); //* Worker thread method for async SLAM processing
